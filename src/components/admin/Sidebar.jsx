...
const SidebarItem = ({ label, icon: Icon, active, onClick, collapsed }) => (
  <button
    className={`flex items-center space-x-4 p-4 w-full text-left ${
  ...
  >
    ...
    {!collapsed && (
      <span
        className={` font-medium ${active ? "text-cyan-500" : "text-gray-300"}`}
      >
        {label}
      </span>
    )}
...
const Sidebar = () => {
  ...
  return (
    <div
      className={`flex flex-col h-screen bg-card   ${
        collapsed ? "w-28" : "w-64"
      }`}
    >
